...
for ($i=0; $row=sql_fetch_array($result); $i++) {
    ...
    $sql3 = " select wr_subject,wr_6 from $tmp_write_table where wr_id = '{$row['wr_id']}' ";
    ...
    $list[$i]['confirm_href'] = './apply_memo_form.php?val=3&ma_id='.$row['ma_id'].'&wr_id='.$row['wr_id'].'&me_recv_mb_id='.$row['mb_id'].'&amp;page='.$page;
    $list[$i]['confrim_refuse_href'] = './apply_memo_form.php?val=4&ma_id='.$row['ma_id'].'&wr_id='.$row['wr_id'].'&me_recv_mb_id='.$row['mb_id'].'&amp;page='.$page;
