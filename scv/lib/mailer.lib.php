...
function mailer($fname, $fmail, $to, $subject, $content, $type=0, $file="", $cc="", $bcc="")
{
    ...
    try {
        ...
        if (defined('G5_SMTP') && G5_SMTP) {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = '네이버 이메일 주소';
            $mail->Password = '네이버 비밀번호';

            # 지메일
            # 1. 아래의 코드를 주석 해제, 위의 코드(네이버)를 주석 처리
            # 2. https://www.google.com/settings/u/1/security/lesssecureapps 
            # 3. https://accounts.google.com/b/0/DisplayUnlockCaptcha
            # 4. https://security.google.com/settings/security/activity?hl=en&pli=1
            // $mail->IsSMTP(); // telling the class to use SMTP
            // $mail->Host = 'smtp.gmail.com'; // SMTP server
            // $mail->SMTPAuth = true;
            // $mail->SMTPSecure = true;
            // $mail->Port = 465;
            // $mail->Username = '지메일 이메일 주소';
            // $mail->Password = '지메일 비밀번호';
        }
        $mail->CharSet = 'UTF-8';
        $mail->From = $fmail;
        $mail->FromName = $fname;
